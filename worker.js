export default {
	...
	async scheduled(controller, env, ctx) {
		...
		const config = initializeConfig(env);
		try {
			const result = await performCheckinWithLogs(config);
			console.log('定时任务执行完成:', result.result);
			await sendMessage(result.result, config);
		} catch (error) {
			console.error('定时任务执行失败:', error);
			await sendMessage(`定时任务执行失败: ${error.message}`, config);
		}
	},
};
...
function initializeConfig(env) {
	let domain = env.JC || env.DOMAIN || "这里填机场域名";
	if (!domain.includes("//")) domain = `https://${domain}`;
	
	return {
		domain: domain,
		user: env.ZH || env.USER || "这里填邮箱",
		pass: env.MM || env.PASS || "这里填机场的真实密码",
		// 👇 这里是新增的面板独立登录密码 (优先读取环境变量 PANEL_PASS，如果没有则默认 123456)
		panelPass: env.PANEL_PASS || "123456", 
...
async function verifyAuth(request, config) {
	...
	// 使用 panelPass 进行身份校验，而不是真实的签到密码
	const rawString = hostname + config.panelPass + ua; 
		...
		switch (pathname) {
			...
			case "/api/info": {
				...
				return Response.json({
					...
					user: mask(config.user, true),
					pass: mask(config.pass), // 面板上显示的是脱敏后的真实机场密码
					tgEnabled: !!(config.BotToken && config.ChatID)
				});
			}

			case "/api/checkin":
				if (request.method !== "POST") return new Response("Method Not Allowed", { status: 405 });
				const res = await performCheckinWithLogs(config);
				return Response.json(res);

			case "/api/test_tg":
				if (request.method !== "POST") return new Response("Method Not Allowed", { status: 405 });
				if (!config.BotToken || !config.ChatID) return Response.json({ success: false, message: "未配置完整的 TG BotToken 和 ChatID" });

				const tgRes = await sendMessage("🔔 这是一条来自动动签到管理面板的测试消息\n如果您能看到这条消息，说明TG推送配置正确！", config);
				if (tgRes && tgRes.ok) {
...
async function performCheckinWithLogs(config) {
	const maxRetries = 3;
	...
	for (let attempt = 1; attempt <= maxRetries; attempt++) {
		try {
			...
			log("访问站点获取初始会话...");
			const initResponse = await fetch(`${config.domain}/auth/login`, {
				method: 'GET',
				headers: {
					'User-Agent': UA,
			...
			log(`请求登录接口: ${config.domain}/auth/login`);
			const loginResponse = await fetch(`${config.domain}/auth/login`, {
				...
				// 实际签到使用真实的 config.pass 密码
				body: JSON.stringify({ email: config.user, passwd: config.pass, remember_me: 'on', code: "" }),
			});
			...
			for (let sc = 1; sc <= maxSessionChecks; sc++) {
				...
				const verifyResponse = await fetch(`${config.domain}/user`, {
					method: 'GET',
					headers: {
						'Cookie': cookieString(allCookies),
						'User-Agent': UA,
			...
			if (checkinResponse.status >= 300 && checkinResponse.status < 400) {
				...
				throw new Error(`签到请求被重定向 (HTTP ${checkinResponse.status}) -> ${location}，Cookie 失效`);
			}
			...
			try {
				...
				throw new Error(`解析签到响应失败: ${responseText.substring(0, 100)}...`);
			}
...
async function sendMessage(msg = "", config) {
	const maskedPass = config.pass.length > 2 ? config.pass[0] + '***' + config.pass[config.pass.length - 1] : '***';
	const 账号信息 = `地址: ${config.domain}\n账号: ${config.user}\n密码: <tg-spoiler>${maskedPass}</tg-spoiler>`;
	...
	if (config.BotToken !== '' && config.ChatID !== '') {
		const url = `https://api.telegram.org/bot${config.BotToken}/sendMessage?chat_id=${config.ChatID}&parse_mode=HTML&text=${encodeURIComponent("执行时间: " + formattedTime + "\n" + 账号信息 + "\n\n" + msg)}`;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body {
      font-family: var(--font); background-color: var(--bg-color); color: var(--text-main);
      min-height: 100vh; display: flex; flex-direction: column; overflow-x: hidden;
      background-image: radial-gradient(circle at 15% 50%, rgba(59, 130, 246, 0.15) 0%, transparent 50%), radial-gradient(circle at 85% 30%, rgba(16, 185, 129, 0.15) 0%, transparent 50%);
    }
    .glass { background: var(--panel-bg); backdrop-filter: blur(16px); border: 1px solid var(--border-color); border-radius: var(--radius); }
    h1, h2, h3 { font-weight: 600; letter-spacing: -0.025em; }
    .hidden { display: none !important; }
    input { width: 100%; padding: 12px 16px; margin-bottom: 16px; background: rgba(15, 23, 42, 0.6); border: 1px solid var(--border-color); color: var(--text-main); border-radius: 8px; font-size: 1rem; transition: all 0.2s; }
    input:focus { outline: none; border-color: var(--accent); box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.2); }
    button { background: var(--accent); color: white; border: none; padding: 12px 20px; border-radius: 8px; font-size: 1rem; font-weight: 500; cursor: pointer; transition: all 0.2s; width: 100%; display: flex; justify-content: center; align-items: center; gap: 8px; }
    button:hover { background: var(--accent-hover); transform: translateY(-1px); }
    button:active { transform: translateY(0); }
    button:disabled { opacity: 0.7; cursor: not-allowed; }
    button.secondary { background: rgba(255,255,255,0.1); }
    button.secondary:hover { background: rgba(255,255,255,0.15); }
    #login-view { display: flex; justify-content: center; align-items: center; flex: 1; padding: 20px; }
    .login-box { width: 100%; max-width: 400px; padding: 40px 40px 24px; text-align: center; animation: fadeIn 0.5s ease-out; }
    ...
    #dashboard-view { padding: 30px; max-width: 1400px; margin: 0 auto; width: 100%; display: grid; grid-template-columns: 350px 1fr; gap: 24px; flex: 1; animation: slideUp 0.4s ease-out; }
    ...
    .status-badge { display: inline-block; padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: 600; background: rgba(16, 185, 129, 0.2); color: var(--success); }
    ...
    .console-header { padding: 16px 24px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center; }
    ...
    .console-body { flex: 1; padding: 20px; overflow-y: auto; font-family: 'JetBrains Mono', 'Fira Code', Consolas, monospace; font-size: 0.9rem; line-height: 1.6; background: rgba(0, 0, 0, 0.3); border-radius: 0 0 var(--radius) var(--radius); }
    ...
    .top-header { padding: 20px 30px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); }
    ...
    .spinner { width: 16px; height: 16px; border: 2px solid rgba(255,255,255,0.3); border-radius: 50%; border-top-color: white; animation: spin 0.8s linear infinite; display: none; }
...
<body>
  <div id="login-view">
    <div class="glass login-box">
      ...
      <p>请输入面板独立密码以访问控制台</p>
      <form id="login-form">
        <input type="password" id="pwd-input" placeholder="输入面板独立密码" required autofocus autocomplete="current-password">
    ...
    <div id="dashboard-view">
      <div class="sidebar">
        <div class="glass info-card">
          ...
          <div class="info-group">
            <div class="info-label">实际签到密码 (已脱敏)</div>
  ...
  <script>
    ...
    function appendLog(message, type = 'info') {
      ...
      let formattedMsg = typeof message === 'object' ? JSON.stringify(message, null, 2) : String(message);
    ...
    function logout() {
      localStorage.removeItem(AUTH_KEY); currentToken = null;
      appView.classList.add('hidden'); loginView.classList.remove('hidden');
      consoleOutput.innerHTML = ''; pwdInput.focus();
    }
    ...
    async function loadDashboardInfo() {
      ...
      if (data.tgEnabled) {
        tgElem.innerHTML = \`<span class="status-badge">✅ 启用 (官方API推送)</span>\`;
      } else {
        tgElem.innerHTML = \`<span class="status-badge disabled">❌ 未配置Bot</span>\`;
      }
    }
